...
// In this coding challenge, we will build a functionality for a command line tool.
// We want to write a function that takes in a non-empty string representing
// a valid Unix-shell path and returns a simplified version of the path.
// The simplified path must be equivalent to the original path,
// that means it should point to same file/directory as the original path.

// A path can be an absolute path, meaning that it starts at the root directory in a file system,
// or a relative path, meaning that it starts at the current directory in a file system.
...
func simplifyPath(path string) string {
	...
	// Use a stack to process the components
	stack := []string{}
	...
	for _, component := range components {
		if component == "." {
			...
			// If stack is empty and it's a relative path, we can't go up further
			// If it's absolute path, going up from root stays at root
		} else {
